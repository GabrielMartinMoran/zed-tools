...
import { getTests, registerTestsChangeObserver } from './states/tests-state';
import { TEST_OUTCOMES, type Test } from './types';

import { $ } from 'bun';
import { extractDiffStrings, type Diff } from './diff-extractor';
import { openDiff, openStream } from './editor-commands';
...
export const buildUI = () => {
    ...
    const displayTestDetails = (tests: Test[], testIndex: number, detailBox: Widgets.BoxElement) => {
    ...
    const setUpEvents = (uiComponents: UIComponents) => {
        ...
        testList.key('s', () => {
            ...
            let nextSkippedIndex = tests.findIndex(
                (test, i) => i > currentIndex && test.outcome === TEST_OUTCOMES.SKIPPED
            );
        ...
        testList.key('f', () => {
            const tests = getTests();
            const currentIndex = testList.selected;
            let nextFailedIndex = tests.findIndex(
                (test, i) => i > currentIndex && test.outcome === TEST_OUTCOMES.FAILED
            );
        ...
        testList.key('p', () => {
            const tests = getTests();
            const currentIndex = testList.selected;
            let nextPassedIndex = tests.findIndex(
                (test, i) => i > currentIndex && test.outcome === TEST_OUTCOMES.PASSED
            );
        ...
        detailBox.on('keypress', async (ch: any, key: any) => {
            ...
            const testName = currentSelectedTest?.nodeid.split('::')[1] as string;
            ...
            if (source && dataToOpen) {
                if (source === 'Diff') {
                    ...
                    await openDiff(testName, dataToOpen);
                    statusBar.setContent(chalk.green.bold(`🚀 ${source} opened in editor!`));
                } else {
                    await openStream(testName, dataToOpen as string, source);
                    statusBar.setContent(chalk.green.bold(`🚀 ${source} opened in editor!`));
                }
            } else if (source) {
